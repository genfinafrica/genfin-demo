...
const FarmerChatbotMock = ({ setView }) => {
    ...
    // RESTORED: State variables for sequential flow control
    ...
    const fetchStatus = async () => {
        try {
            ...
            // FIX: Using farmer name from status, falling back to mock name
            let statusMessage = `✅ Status for ${response.data.name || 'Aisha Abdalla'}:\n`;
    ...
    const handleInput = async (e) => {
        ...
        // RESTORED: Core Flow Control Logic
        ...
        if (showUploadInput) {
            ...
            setShowUploadInput(false); // Reset upload mode after input is processed
            return;
        }

        if (showIoTInput) {
            handleIotData(inputTrimmed);
            setShowIoTInput(false); // Reset IoT mode after input is processed
            return;
        }
        // END RESTORED LOGIC
        ...
        switch (command) {
            ...
            case 'REGISTER': // Allow re-registration prompt if needed
    ...
    const initiateUpload = async () => {
        ...
        const currentStatus = farmerStatus;
    ...
    const handleFileUpload = async (fileInput) => {
        ...
        if (!fileName || !fileType) {
            ...
            // Re-enable upload input to allow retry without re-typing UPLOAD
            setShowUploadInput(true); 
            return;
        }
        ...
        try {
            ...
            // Re-enable upload input to allow retry
            setShowUploadInput(true); 
        }
    };
    ...
    const handleRegistration = async (input) => {
        // Allow exiting registration by typing 'STATUS' to use default farmer ID 1
    ...
    const handleIotData = async (dataInput) => {
        ...
        try {
            ...
            setShowIoTInput(false);
             ...
             // If ingestion fails, allow retry
             setShowIoTInput(true);
        }
    };
