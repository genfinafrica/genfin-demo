...
const FarmerChatbotMock = ({ setView }) => {
    ...
    const handleRegistration = async () => {
        const payload = {
            ...
            land_size: parseFloat(farmerData.land_size) // Ensure float
        };
    ...
    const handleUpload = async (stageNumber, fileType, fileName) => {
        if (fileType === 'soil_test' && !fileName.toLowerCase().endswith('.csv')) {
            addMessage('BOT', 'File name must end with .csv for soil test.');
            setShowMockFileInput(true);
            return;
        }
        if (fileType === 'photo_evidence' && !fileName.toLowerCase().endswith(('.jpg', '.jpeg', '.png'))) {
            addMessage('BOT', 'File name must end with .jpg, .jpeg, or .png for photo evidence.');
            setShowMockFileInput(true);
            return;
        }
        try {
    ...
    const handleStatus = async () => {
        ...
        while (attempts < maxAttempts) {
            try {
                const statusResponse = await axios.get(`${API_BASE_URL}/api/farmer/${farmerId}/status`);
                console.log(`Status response for farmer ${farmerId}: ${JSON.stringify(statusResponse.data)}`);
                ...
                attempts++;
                console.error(`Status fetch attempt ${attempts} failed for farmer ${farmerId}:`, error.response?.data || error);
                if (attempts === maxAttempts) {
                    addMessage('BOT', `❌ Failed to fetch status after ${maxAttempts} attempts. Are you sure your ID is correct?`);
                    setFlowState('GET_ID'); // Stay in GET_ID to prompt for ID again
                }
    ...
    const handleInput = (e) => {
        ...
        if (flowState === 'INTRO') {
            if (text.toUpperCase() === 'REGISTER') {
                botMessage("Great! Let's get you set up. What is your full name?");
                ...
                botMessage("Please enter your Farmer ID to upload files.");
                setFlowState('UPLOAD_GET_ID');
            } else {
                botMessage("Please type 'REGISTER', 'STATUS', or 'UPLOAD'.");
            }
        ...
        } else if (flowState === 'UPLOAD_GET_ID') {
            const id = parseInt(text);
            if (!isNaN(id)) {
                setFarmerId(id);
                axios.get(`${API_BASE_URL}/api/farmer/${id}/status`).then(res => {
                    ...
                    if (nextStage.stage_number === 1) {
                        botMessage("Type a mock soil test file name (e.g., soil_test.csv) in the chat input.");
                        setFlowState('UPLOAD_SOIL_TEST');
                        setShowMockFileInput(true);
                    } else if (nextStage.stage_number in [3, 4]) {
                        botMessage(`Type a mock photo file name for Stage ${nextStage.stage_number} (e.g., photo.jpg) in the chat input.`);
            ...
            if (isNaN(landSize) || landSize < 0) {
                botMessage("Invalid land size. Please enter a non-negative number (e.g., 5.0).");
            } else {
