...
const formatBotMessage = (text) => {
    ...
    const regex = /(STATUS|NEXT STAGE|UPLOAD|REGISTER|CANCEL)/g;
...
const FarmerChatbotMock = ({ setView }) => {
    ...
    const [uploadMode, setUploadMode] = useState(false); // New state to track if we're waiting for upload input
    const [iotMode, setIotMode] = useState(false);     // New state to track if we're waiting for IoT input
    const [registrationMode, setRegistrationMode] = useState(true); // New state to track if we're waiting for registration input
    ...
    const fetchStatus = async () => {
        try {
            ...
            let statusMessage = `✅ Status for ${response.data.name}:\n`;
    ...
    const handleInput = async (e) => {
        ...
        const command = inputTrimmed.toUpperCase();
        pushUserMessage(inputTrimmed);
        setInput('');

        // --- Core Flow Control Fix ---
        if (registrationMode) {
            // FIX: This ensures the first input always attempts registration.
            handleRegistration(inputTrimmed);
            return;
        }

        if (uploadMode) {
            handleFileUpload(inputTrimmed);
            setUploadMode(false);
            return;
        }

        if (iotMode) {
            handleIotData(inputTrimmed);
            setIotMode(false);
            return;
        }
        // --- End Core Flow Control Fix ---
        ...
        switch (command) {
            ...
            case 'REGISTER':
                // For users who missed the first prompt and already have a farmer ID
                if (farmerId !== 1) {
                    pushBotMessage("You are already registered as Farmer ID " + farmerId + ". Type **STATUS**.");
                    return;
                }
                setRegistrationMode(true);
                pushBotMessage("Please enter Registration details: Name, Phone, Crop, Land Size (e.g., Aisha Abdalla, 254711223344, Maize, 5.0).");
                break;
    ...
    const initiateUpload = async () => {
        if (!farmerStatus) {
            // Attempt to fetch status if not already available
            await fetchStatus(); 
            // Give the fetch a moment to update state before checking again
            if (!farmerStatus) { 
                 pushBotMessage("Please check **STATUS** first to load your profile data.");
                 return;
            }
        }
        
        // Use the most up-to-date status if possible
        const currentStatus = farmerStatus || (await axios.get(`${API_BASE_URL}/api/farmer/${farmerId}/status`)).data;
        setFarmerStatus(currentStatus);

        const nextStage = currentStatus.stages.find(s => s.status === 'UNLOCKED' || s.status === 'PENDING');
        ...
        if (nextStage.stage_number === 5) {
            if (currentStatus.current_status.pest_flag) {
                // If pest flag is active, treat stage 5 as unlocked for upload
                const formatInfo = stageFileFormats[nextStage.stage_number] || '.jpg, .pdf'; // Stage 5 is conditional, fallback to photo
                setUploadMode(true);
                pushBotMessage(`Stage 5: Pest/Disease (Conditional) is UNLOCKED due to a pest event. Please submit the **Inputs/Invoice Photo** for this disbursement. Acceptable formats: ${formatInfo}. Type 'CANCEL' to abort.`);
                return;
            } else {
    ...
    const handleFileUpload = async (fileInput) => {
        if (fileInput.toUpperCase() === 'CANCEL') {
        ...
        if (!fileName || !fileType) {
            ...
            // Keep upload mode true to allow retry, or change to false to require UPLOAD command again
            setUploadMode(true); 
            return;
        }
        ...
        if (!nextStage) {
             pushBotMessage("Upload stage not found. Please type **STATUS**.");
             return;
        }
        ...
        try {
            ...
            // Keep upload mode true to allow retry
            setUploadMode(true); 
        }
    };
    ...
    const handleRegistration = async (input) => {
        if (input.toUpperCase() === 'STATUS' || input.toUpperCase() === 'UPLOAD' || input.toUpperCase() === 'NEXT STAGE') {
             // If the user attempts to use a command without registering, let them through to use Farmer ID 1.
             setRegistrationMode(false);
             handleInput(new Event('submit')); // Re-process the input as a command
             return;
        }
    ...
    useEffect(() => {
        ...
        pushBotMessage(`Welcome to the GENFIN 🌱 Africa Demo. Please **REGISTER** to start or type **STATUS** to use the default Farmer ID 1 (Aisha Abdalla).`);
        // If Farmer ID 1 exists, we allow command flow without explicitly registering
        setRegistrationMode(true); 
    }, []);
