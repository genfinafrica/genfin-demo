...
const FarmerChatbotMock = ({ setView }) => {
    ...
    const fetchStatus = async (id = farmerId) => {
        ...
        try {
            ...
            statusMessage += `💧 Drought Flag: ${data.current_status?.drought_flag ? '⚠️ ACTIVE' : 'No'}\n\n`;
            ...
            if (data.has_insurance) {
                ...
                if (data.insurance_triggered) {
                    if (data.claim_status === 'PENDING') {
                        statusMessage += `   Claim: Pending Review\n`;
                    } else if (data.claim_status === 'APPROVED') {
                        statusMessage += `   Claim: Approved\n`;
                    } else if (data.claim_status === 'REJECTED') {
                        statusMessage += `   Claim: Rejected\n`;
                    }
                } else {
                    statusMessage += `   No active claims\n`;
                }
            } else {
                statusMessage += `\n🛡️ Insurance: Not Active\n`;
            }
            ...
            // Check for IoT prompt after Stage 4
            const stage4 = stages.find(s => s.stage_number === 4);
            if (stage4?.status === 'COMPLETED' && data.has_insurance && !data.insurance_triggered && !showIoTInput) {
                pushBotMessage("🌦️ Stage 4 completed! Time to check your soil sensor readings. Type **IOT** to upload your data for insurance evaluation.");
            }
        } catch (error) {
    ...
    const handleIotData = async (dataInput) => {
        ...
        try {
            ...
            if (resp.data.drought_flag) {
                pushBotMessage("💧 Drought risk detected. Insurance claim has been filed for review by the insurer dashboard.");
            } else {
                pushBotMessage("✅ Moisture levels are adequate. No claim required.");
            }
            await fetchStatus();
            return;
        } catch (err) {
...
const InsurerDashboard = ({ setView }) => {
    ...
    const handleClaimReview = async (action) => {
        if (!selectedFarmerId) return;
        try {
            const response = await axios.post(`${API_BASE_URL}/api/insurer/review/${selectedFarmerId}`, { action });
            alert(response.data.message);
            await fetchFarmerDetails(selectedFarmerId);
        } catch (error) {
            alert(error.response?.data?.message || "Claim review failed.");
        }
    };
    useEffect(() => {
        fetchInsurerFarmers();
    ...
    // RENDER DETAILED VIEW
    let claimState = 'No active claims';
    let claimColor = 'gray';
    if (farmerData.insurance_triggered) {
        if (farmerData.claim_status === 'PENDING') {
            claimState = 'Pending Review';
            claimColor = 'orange';
        } else if (farmerData.claim_status === 'APPROVED') {
            claimState = 'Approved';
            claimColor = 'green';
        } else if (farmerData.claim_status === 'REJECTED') {
            claimState = 'Rejected';
            claimColor = 'red';
        }
    }
    return (
        <div className="dashboard-list-container">
            ...
            <div className="policy-card">
                ...
                {!farmerData.policy_id ? (
                    ...
                    <>
                        ...
                        <p>Status:
                            <span className={`policy-status-${farmerData.has_insurance ? 'active' : 'pending'}`}>
                                <strong>{farmerData.has_insurance ? 'ACTIVE' : 'PENDING'}</strong>
                            </span>
                        </p>
                        <p>Claim Status: <span style={{ color: claimColor }}>{claimState}</span></p>
            ...
            <div style={{ margin: '20px 0' }}>
                ...
                <button className="btn-insurer" onClick={handleBindPolicy} disabled={farmerData.has_insurance}>
                    Bind Policy (Manual)
                </button>
                {farmerData.claim_status === 'PENDING' && (
                    <>
                        <button className="btn-insurer" onClick={() => handleClaimReview('approve')}>
                            Approve Claim
                        </button>
                        <button className="btn-insurer" onClick={() => handleClaimReview('reject')}>
                            Reject Claim
                        </button>
                    </>
                )}
