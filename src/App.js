...
const FarmerChatbotMock = ({ setView }) => {
    ...
    const fetchStatus = async (id = farmerId) => {
        ...
        try {
            ...
            statusMessage += `🛡️ Insurance: ${data.current_status?.has_insurance ? 'Active' : 'Pending'}\n`;
            if (data.current_status?.insurance_triggered) {
                statusMessage += ` | Claim: ${data.current_status?.claim_status || 'Pending'}\n`;
            }
    ...
    const handleIotData = async (dataInput) => {
        ...
        try {
            ...
            const resp = await axios.post(`${API_BASE_URL}/api/iot/ingest`, {...parsed, farmer_id: farmerId});
...
const InsurerDashboard = ({ setView }) => {
            ...
            <div className="dashboard-list-container">
                ...
                {farmers.map((farmer) => (
                    <div key={farmer.id} className="farmer-card">
                        <div>
                            <strong>{farmer.name} (ID: {farmer.id})</strong><br/>
                            <span>Policy: {farmer.policy_status} | Claim: {farmer.claim_status || 'None'} | Score: {farmer.score}</span>
                        </div>
    ...
    // RENDER DETAILED VIEW
    const policyStatus = farmerData.policy_status || 'PENDING';
    const claimStatus = farmerData.claim_status ? farmerData.claim_status : 'No active claim';
    return (
        <div className="dashboard-list-container">
            ...
            <div style={{ margin: '20px 0' }}>
                ...
                {claimStatus === 'PENDING' && (
                    <>
                        <button className="btn-insurer" onClick={() => handleReview('approve')}>
                            Approve Claim
                        </button>
                        <button className="btn-insurer" onClick={() => handleReview('reject')}>
                            Reject Claim
                        </button>
                    </>
                )}
